...
generate_type = {'iv_s':['train', 'data/in_vocab.subset.tro.37'],
                ...
                # hieroglyphs: style refs under data/hiero/IAM64-new/test, corpus of single signs
                'hiero':['test', 'data/hiero_corpus.txt']}
...
class IAMDataset(Dataset):
    def __init__(self, image_path, style_path, laplace_path, type, content_type='unifont', max_len=9,
                 letters_path='', text_txt=None):
        # letters_path / text_txt let a non-Latin dataset (e.g. hieroglyphs built by
        # pipelines/generation/prep_dataset.py) override the module-level Latin charset
        # and the hard-coded IAM64 index files; the defaults keep IAM behaviour.
        self.max_len = max_len
        self.style_len = style_len
        self.data_dict = self.load_data(text_txt if text_txt else text_path[type])
    ...
    def collate_fn_(self, batch):
        ...
        # content bitmap size follows the loaded content pickle (16x16 for Latin
        # unifont; 32x32 for the hieroglyph content built by prep_dataset.py --
        # both collapse to the same 512-d token in Mix_TR.content_encoder)
        content_ref = torch.zeros([len(batch), max(c_width), self.con_symbols.shape[1], self.con_symbols.shape[2]], dtype=torch.float32)
...
class ContentData(IAMDataset):
    def __init__(self, content_type='unifont', letters_path='') -> None:
        # letters_path lets a non-Latin charset (e.g. hieroglyphs) drive the content
        # stack; default '' keeps the module-level Latin `letters` (IAM behaviour).
        if letters_path:
            with open(letters_path, 'r', encoding='utf-8') as f:
                self.letters = f.read().strip()
        else:
